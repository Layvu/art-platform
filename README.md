...
Методы вызываются внутри кастомных эндпоинтов. Перед их вызовом эндпоинт проверяет user (его роль, права, существование) - основная сущность пользователя в payloadCMS. Сами серверные сервисы используют токен главного администратора (`PAYLOAD_API_KEY`) для выполнения манипуляций с БД
